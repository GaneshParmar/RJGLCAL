<script lang="ts">
	...
	import { calculateDiffBetweenDates, isValidateDates } from '$lib/utils/DateUtilis';
	import CustomSelect from './CustomSelect.svelte';
	import CalculationModal from './CalculationModal.svelte';
	import AvatarMessage from '$lib/common/AvatarMessage.svelte';
	import type { Alert_ } from '$lib/types/AlertTypes';
	...
	function calculate(e: Event) {
		...
		// validating end-date and start-date

		const isValid = isValidateDates(endDate, startDate);
		...
		if (isValid) {
			...
			isInValidDates = true;
			alert.message = "End Date can't be smaller than start date. Loan की समाप्ति तिथि को Loan की शुरुआत की तारीख से बाद या उसके बराबर होना चाहिए।";
			alert.type = 'red';
...
<AvatarMessage {alert} bind:open={isInValidDates} />
